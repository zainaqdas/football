...
const $$EventCard = createComponent(($$result, $$props, $$slots) => {
  ...
  const { description, hour, date, country, embeds = [], index = 0, locale = "en" } = Astro2.props;
  const delay = index * 80;
  const isSpanish = locale === "es";
  return renderTemplate`${maybeRenderHead()}<div class="bg-canvas rounded-lg card-hairline p-5 transition-all duration-300 hover:shadow-elevated hover:-translate-y-0.5"${addAttribute(`animation-delay: ${delay}ms`, "style")}> <div class="flex items-start gap-4"> <!-- Time column --> <div class="shrink-0 w-16 text-center"> <div class="text-lg font-semibold text-ink leading-tight">${hour || "--:--"}</div> ${date && renderTemplate`<div class="text-[10px] font-mono text-mute uppercase tracking-wider mt-1">${date}</div>`} </div> <!-- Content --> <div class="flex-1 min-w-0"> <div class="flex items-center gap-2 mb-2"> ${country?.flag?.url && renderTemplate`<img${addAttribute(country.flag.url, "src")}${addAttribute(country.name || "", "alt")} class="w-5 h-4 rounded object-cover" loading="lazy">`} ${country?.name && renderTemplate`<span class="text-xs font-mono text-mute uppercase tracking-wider">${country.name}</span>`} </div> <h3 class="text-sm font-medium text-ink leading-snug"> ${description || (isSpanish ? "Evento sin nombre" : "Unnamed event")} </h3> <!-- Embeds - clickable to our player when streamId is found --> ${embeds.length > 0 && renderTemplate`<div class="flex flex-wrap gap-1.5 mt-3"> ${embeds.map((embed) => {
