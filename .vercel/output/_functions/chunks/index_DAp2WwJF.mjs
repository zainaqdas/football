...
View Schedule
</a> </div> </div> </div> <!-- Gradient fade at bottom --> <div class="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-canvas-soft to-transparent"></div> </section>  <section class="bg-canvas-soft py-16 sm:py-20"> <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"> <div class="grid grid-cols-2 md:grid-cols-4 gap-4 stagger-children"> ${renderComponent($$result2, "StatsCard", $$StatsCard, { "label": "Total Channels", "value": channelStats.totalChannels, "icon": "channels" })} ${renderComponent($$result2, "StatsCard", $$StatsCard, { "label": "Live Channels", "value": channelStats.totalActive, "change": `${Math.round(channelStats.totalActive / channelStats.totalChannels * 100)}%`, "icon": "active" })} ${renderComponent($$result2, "StatsCard", $$StatsCard, { "label": "Categories", "value": channelStats.totalCategories, "icon": "categories" })} ${renderComponent($$result2, "StatsCard", $$StatsCard, { "label": "Events Today", "value": eventStats.totalEvents, "change": `${eventStats.uniqueCountries.length} países`, "icon": "events" })} </div> </div> </section>  <section class="bg-canvas py-16 sm:py-20"> <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"> <div class="text-center mb-12"> <p class="text-xs font-mono text-mute uppercase tracking-widest mb-3">Categories</p> <h2 class="text-3xl sm:text-4xl font-semibold text-ink tracking-[-1.28px]">
Explore by region.
