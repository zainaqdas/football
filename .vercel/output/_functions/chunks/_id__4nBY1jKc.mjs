...
import 'piccolore';
import { k as renderTemplate, h as addAttribute, m as maybeRenderHead, o as renderComponent, p as Fragment } from './entrypoint_Bkn4krmA.mjs';
...
const $$VideoPlayer = createComponent(($$result, $$props, $$slots) => {
  ...
  const { hlsUrl, channelName = "Stream", class: className = "", locale = "en" } = Astro2.props;
  ...
  const liveText = isSpanish ? "En Vivo" : "Live";
  const errorText = isSpanish ? "Error al cargar el stream" : "Error loading stream";
  const retryText = isSpanish ? "Reintentar" : "Retry";
  const unavailableText = isSpanish ? "Stream no disponible" : "Stream unavailable";
  const unavailableSubtext = isSpanish ? "No hay una fuente de video disponible para este canal." : "No video source available for this channel.";
  const notSupportedText = isSpanish ? "Este navegador no soporta HLS." : "This browser does not support HLS playback.";
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('[id^="player-"][data-proxied-url]').forEach(function (container) {
      ...
      var liveText = container.getAttribute('data-live') || 'Live';
      var notSupportedText = container.getAttribute('data-not-supported') || 'This browser does not support HLS playback.';
      ...
      // Set live badge text from data attribute
      if (controls.liveBadge) {
        var dot = controls.liveBadge.querySelector('span');
        controls.liveBadge.textContent = '';
        if (dot) controls.liveBadge.appendChild(dot);
        controls.liveBadge.appendChild(document.createTextNode(liveText));
      }
      ...
      function initPlayer(url) {
        if (window.Hls && window.Hls.isSupported()) {
          ...
          controls.loadingSpinner.classList.add('hidden');
          controls.errorOverlay.querySelector('p').textContent = notSupportedText;
          controls.errorOverlay.classList.remove('hidden');
        }
      }
...
const $$id = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const title = channel ? t("channel.pageTitleLive", { name: channel.name, site: "Fútbol Libre TV" }) : t("channel.pageTitleNotFound", { site: "Fútbol Libre TV" });
  const description = channel ? t("channel.pageDesc", { name: channel.name, category: channel.category }) : t("channel.pageDescNotFound");
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": title, "description": description, "locale": locale }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<section class="bg-canvas min-h-[calc(100vh-4rem)]"> <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8"> ${channel ? renderTemplate`${renderComponent($$result2, "Fragment", Fragment, {}, { "default": async ($$result3) => renderTemplate`  <nav class="flex items-center gap-2 text-sm text-mute mb-6 font-mono"> <a href="/channels" class="text-mute no-underline hover:text-ink transition-colors">${t("channel.breadcrumbChannels")}</a> <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m9 18 6-6-6-6"></path></svg> <a${addAttribute(`/channels/${channel.category.toLowerCase()}`, "href")} class="text-mute no-underline hover:text-ink transition-colors">${channel.category}</a> <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m9 18 6-6-6-6"></path></svg> <span class="text-ink">${channel.name}</span> </nav>  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8"> <!-- Main content --> <div class="lg:col-span-2"> ${renderComponent($$result3, "VideoPlayer", $$VideoPlayer, { "hlsUrl": hlsUrl, "channelName": channel.name, "locale": locale })} <!-- Channel info --> <div class="mt-6"> <div class="flex items-start justify-between gap-4"> <div> <div class="flex items-center gap-3 mb-2"> <h1 class="text-2xl font-semibold text-ink tracking-[-0.6px]">${channel.name}</h1> ${channel.isActive ? renderTemplate`<span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-green-50 text-green-700 text-xs font-mono font-medium rounded-full"> <span class="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></span>${t("channel.live")} </span>` : renderTemplate`<span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-gray-50 text-gray-400 text-xs font-mono font-medium rounded-full"> <span class="w-1.5 h-1.5 rounded-full bg-gray-300"></span>${t("channel.inactive")} </span>`} </div> <p class="text-sm text-mute font-mono">${channel.category}</p> </div> </div> ${streamInfo && hlsUrl && renderTemplate`<div class="mt-6 p-4 bg-green-50 border border-green-100 rounded-lg"> <div class="flex items-center gap-2 text-sm text-green-800"> <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"> <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline> </svg> <span class="font-medium">${t("channel.playingDirectly")}</span> <span class="text-green-600 text-xs ml-auto">${t("channel.livePlayback")}</span> </div> </div>`} ${streamInfo && renderTemplate`<details class="group mt-6"> <summary class="text-sm font-mono text-mute cursor-pointer hover:text-body transition-colors inline-flex items-center gap-2"> <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polyline points="9 18 15 12 9 6"></polyline></svg> ${t("channel.techInfo")} </summary> <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4"> ${streamInfo.hlsStreams.length > 0 && renderTemplate`<div class="bg-canvas-soft rounded-lg p-4"> <p class="text-xs font-mono text-mute uppercase tracking-wider mb-2">${t("channel.hlsStreams")}</p> <div class="space-y-1.5"> ${streamInfo.hlsStreams.map((hls, i) => renderTemplate`<div${addAttribute(i, "key")} class="text-xs text-body truncate font-mono">${hls}</div>`)} </div> </div>`} ${streamInfo.iframes.length > 0 && renderTemplate`<div class="bg-canvas-soft rounded-lg p-4"> <p class="text-xs font-mono text-mute uppercase tracking-wider mb-2">${t("channel.embeds")}</p> <div class="space-y-1.5"> ${streamInfo.iframes.map((iframe, i) => renderTemplate`<div${addAttribute(i, "key")} class="text-xs text-body truncate"> <span class="font-mono text-mute">#${i + 1}:</span> ${iframe.src} </div>`)} </div> </div>`} </div> </details>`} </div> </div> <!-- Sidebar --> <div class="lg:col-span-1"> <div class="bg-canvas rounded-lg card-hairline p-5"> <h3 class="text-xs font-mono text-mute uppercase tracking-wider mb-4">${t("channel.info")}</h3> <div class="space-y-3"> <div class="flex justify-between text-sm"> <span class="text-mute">${t("channel.channelLabel")}</span> <span class="font-medium text-ink">${channel.name}</span> </div> <div class="flex justify-between text-sm"> <span class="text-mute">${t("channel.region")}</span> <span class="font-medium text-ink">${channel.category}</span> </div> <div class="flex justify-between text-sm"> <span class="text-mute">${t("channel.status")}</span> <span${addAttribute(`font-medium ${channel.isActive ? "text-green-600" : "text-mute"}`, "class")}> ${channel.isActive ? t("channel.live") : t("channel.inactive")} </span> </div> <div class="border-t border-hairline my-2"></div> <div class="flex justify-between text-sm"> <span class="text-mute">${t("channel.quality")}</span> <span class="font-mono text-xs text-ink">HD</span> </div> <div class="flex justify-between text-sm"> <span class="text-mute">${t("channel.protocol")}</span> <span class="font-mono text-xs text-ink">HLS</span> </div> ${hlsUrl && renderTemplate`<div class="flex justify-between text-sm"> <span class="text-mute">${t("channel.playback")}</span> <span class="font-mono text-xs text-green-600">${t("channel.direct")}</span> </div>`} ${streamInfo && renderTemplate`${renderComponent($$result3, "Fragment", Fragment, {}, { "default": async ($$result4) => renderTemplate` <div class="border-t border-hairline my-2"></div> <div class="flex justify-between text-sm"> <span class="text-mute">HTML</span> <span class="font-mono text-xs text-mute">${(streamInfo.rawHtmlSize / 1024).toFixed(1)} KB</span> </div> <div class="flex justify-between text-sm"> <span class="text-mute">Scripts</span> <span class="font-mono text-xs text-mute">${streamInfo.scripts.length}</span> </div> <div class="flex justify-between text-sm"> <span class="text-mute">Iframes</span> <span class="font-mono text-xs text-mute">${streamInfo.iframes.length}</span> </div> <div class="flex justify-between text-sm"> <span class="text-mute">HLS</span> <span class="font-mono text-xs text-mute">${streamInfo.hlsStreams.length}</span> </div> ` })}`} </div> </div> </div> </div> ` })}` : renderTemplate`<div class="text-center py-20"> <div class="w-16 h-16 mx-auto mb-6 rounded-2xl bg-canvas-soft-2 flex items-center justify-center"> <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" class="text-mute"> <circle cx="12" cy="12" r="10"></circle><path d="m15 9-6 6"></path><path d="m9 9 6 6"></path> </svg> </div> <h3 class="text-lg font-semibold text-ink mb-2">${t("channel.notFound")}</h3> <p class="text-sm text-mute mb-6">${t("channel.notFoundSubtitle")}</p> <a href="/channels" class="inline-flex items-center h-10 px-6 bg-ink text-white text-sm font-medium rounded-pill no-underline hover:bg-ink/90 transition-colors"> ${t("channel.browseChannels")} </a> </div>`} </div> </section> ` })}`;
