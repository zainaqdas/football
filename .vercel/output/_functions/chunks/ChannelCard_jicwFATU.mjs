...
import 'piccolore';
import { m as maybeRenderHead, h as addAttribute, k as renderTemplate } from './entrypoint_DlJc2ilo.mjs';
import 'clsx';
...
const $$ChannelCard = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<a${addAttribute(`/stream/${streamId}`, "href")} class="group block bg-canvas rounded-lg card-hairline p-5 no-underline transition-all duration-300 hover:-translate-y-0.5 hover:shadow-elevated"${addAttribute(`animation-delay: ${delay}ms`, "style")}> <div class="flex items-start justify-between gap-3 mb-3"> <div class="flex-1 min-w-0"> <h3 class="text-sm font-semibold text-ink truncate group-hover:text-link transition-colors"> ${name} </h3> <p class="text-xs text-mute mt-0.5 font-mono uppercase tracking-wider truncate"> ${category} </p> </div> <span${addAttribute(`shrink-0 inline-flex items-center gap-1.5 px-2 py-0.5 text-[11px] font-mono font-medium rounded-full ${isActive ? "bg-green-50 text-green-700" : "bg-gray-50 text-gray-400"}`, "class")}> <span${addAttribute(`w-1.5 h-1.5 rounded-full ${isActive ? "bg-green-500 animate-pulse" : "bg-gray-300"}`, "class")}></span> ${isActive ? "En vivo" : "Inactivo"} </span> </div> <div class="flex items-center gap-2 text-xs text-mute font-mono"> <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="shrink-0"> <polygon points="5 3 19 12 5 21 5 3"></polygon> </svg> <span>${isActive ? "Transmitiendo en vivo" : "Sin transmisión"}</span> </div> <!-- Hover indicator --> <div class="mt-3 pt-3 border-t border-hairline opacity-0 group-hover:opacity-100 transition-opacity"> <span class="text-xs font-medium text-link flex items-center gap-1">
Ver stream
