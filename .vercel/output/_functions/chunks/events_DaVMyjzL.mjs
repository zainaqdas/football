...
Pantalla completa
  ...
  function setupModalPlayer(container, modalContainer) {
    ...
    // Grab control references from the player HTML
    var c = {
    ...
    // Cleanup on close via global reference
  ...
  window.openEventPlayer = async function (streamId, channelName, category) {
      ...
      try {
        ...
        // Create the custom player shell (reusing VideoPlayer's structure)
        ...
        // Kick off the custom player for this modal player
        setTimeout(function () {
    ...
    // Intercept embed clicks to open modal player
